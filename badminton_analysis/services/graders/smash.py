...
class SmashGrader(Grader):
    ...
    def __init__(self, handedness: Handedness):
        ...
        SmashGrader.mean = pd.read_csv(self.data_dir / "mean.csv", index_col=0).set_index("feature")
        SmashGrader.std = pd.read_csv(self.data_dir / "std.csv", index_col=0).set_index("feature")
        SmashGrader.mean.columns = [0, 1, 2, 3, 4]
        SmashGrader.std.columns = [0, 1, 2, 3, 4]
        ...
        if rot_path.exists():
            ...
            if hip_mean <= 45.0:
                SmashGrader.hip_rotation_mean = hip_mean
                SmashGrader.hip_rotation_std = hip_std
            else:
                SmashGrader.hip_rotation_mean = None
                SmashGrader.hip_rotation_std = None
            SmashGrader.shoulder_rotation_mean = float(rot.loc["Shoulder Line Rotation CP6", "mean"])
            SmashGrader.shoulder_rotation_std = float(rot.loc["Shoulder Line Rotation CP6", "std"])
        else:
            SmashGrader.hip_rotation_mean = None
            SmashGrader.hip_rotation_std = None
            SmashGrader.shoulder_rotation_mean = None
            SmashGrader.shoulder_rotation_std = None
    ...
    def grade_checkpoint_2(
        ...
        # Angle of the vector from dominant hip to non-dominant hip
        ...
        hip_mean = SmashGrader.hip_rotation_mean
        hip_std = SmashGrader.hip_rotation_std
    ...
    def grade_checkpoint_6(
        ...
        rotation_deg = abs((end_angle - start_angle + 180) % 360 - 180)
        shoulder_mean = SmashGrader.shoulder_rotation_mean
        shoulder_std = SmashGrader.shoulder_rotation_std
